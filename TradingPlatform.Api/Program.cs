...
// ===== Serilog bootstrap logger (Phase 8) =====
// Captures anything that happens before the host's DI container is built
// (e.g. configuration errors during builder setup). Replaced below by the
// fully configured logger once appsettings.json is available.
...
try
{
    Log.Information("Starting TradingPlatform.Api");

    var builder = WebApplication.CreateBuilder(args);

    // ===== Serilog (Phase 8) =====
    // Replaces the default ASP.NET Core logging provider. Reads level
    // overrides from the "Serilog" section of appsettings.json (falls back
    // to sane defaults if that section is absent) and routes ASP.NET's own
    // internal log messages through the same console sink as application logs.
    ...
    // Digest handshake is implemented MANUALLY in AuthService (MD5 only) instead of
    // relying on HttpClientHandler's built-in Digest support.
    ...
    // package needed here; only the @microsoft/signalr JS client is added on the
    // frontend (Phase 6).
    ...
    // Vite dev ports covered — matches the trader-web dev server (Phase 6).
    ...
    // ===== Serilog request logging (Phase 8) =====
    // Condenses ASP.NET's normally-noisy per-request log events into a single
    // structured line per HTTP request (method, path, status, elapsed ms).
...
{
    Log.Fatal(ex, "TradingPlatform.Api terminated unexpectedly");
}
finally
{
    Log.CloseAndFlush();
